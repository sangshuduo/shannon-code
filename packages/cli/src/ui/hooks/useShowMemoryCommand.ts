...
import { debugLogger, type Config } from '@sangshuduo/shannon-core';
  ...
  return async () => {
    ...
    if (currentMemory && currentMemory.trim().length > 0) {
      ...
      addMessage({
        ...
        content:
          fileCount > 0
            ? 'Hierarchical memory (SHANNON.md or other context files) is loaded but content is empty.'
            : 'No hierarchical memory (SHANNON.md or other context files) is currently loaded.',
        timestamp: new Date(),
      });
    }
  };
