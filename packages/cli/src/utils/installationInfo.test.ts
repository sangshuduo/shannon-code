...
import { isGitRepository, debugLogger } from '@sangshuduo/shannon-core';

vi.mock('@sangshuduo/shannon-core', async (importOriginal) => {
  const actual =
    await importOriginal<typeof import('@sangshuduo/shannon-core')>();
  return {
    ...actual,
...
describe('getInstallationInfo', () => {
  ...
  it('should detect global pnpm installation', () => {
    const pnpmPath = `/Users/test/.pnpm/global/5/node_modules/.pnpm/some-hash/node_modules/shannon-code/dist/index.js`;
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('pnpm add -g shannon-code@latest');
  ...
  it('should detect global yarn installation', () => {
    const yarnPath = `/Users/test/.yarn/global/node_modules/shannon-code/dist/index.js`;
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('yarn global add shannon-code@latest');
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should detect global bun installation', () => {
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('bun add -g shannon-code@latest');
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should default to global npm installation for unrecognized paths', () => {
    ...
    expect(info.packageManager).toBe(PackageManager.NPM);
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('npm install -g shannon-code@latest');
    expect(info.updateMessage).toContain('Attempting to automatically update');
