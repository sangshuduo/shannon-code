...
- `shannon-core`: The backend, handling logic and tool execution.
- `shannon-code`: The user-facing frontend.
...
- **NPM publication:** For publishing to the NPM registry, the TypeScript source
  code in `shannon-core` and `shannon-code` is transpiled into standard
  JavaScript using the TypeScript Compiler (`tsc`). The resulting `dist/`
  directory is what gets published in the NPM package. This is a standard
